...
// 模块级纯函数（只读 store getState）：引用稳定，addFiles 的 useCallback 依赖链不被打破
...
export function useChatAttachments(args: {
  ...
  const addOneFile = useCallback(
    ...
    [onError, sessionIdRef, commitAttachments, pushAttachment],
  );
   ...
   *  useCallback 稳定引用：ChatView 的 chatPendingFiles 消费 effect 依赖它 */

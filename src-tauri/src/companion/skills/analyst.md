...
- 已有条目已覆盖的事实，不要重复输出
- 同一件事的另一天样本（今天连这台服务器、明天连那台；今天改这个文件、明天改那个）
  不是新事实。能让已有条目更准/更全就 update 覆盖它（target_id 填它的 id），否则跳过
...
- 可从数据直接查到的（重复且会过期）
- 临时任务状态（上午在改xxx文件、正在写某个功能、项目当前进展到哪一步）
- 实例级细节：具体文件/类/函数名、IP 地址、视频或直播标题、文档名——实例是会过期的
  样本，要沉淀的是抹掉实例后的稳定模式（✗「他在调试 PreAuthorizeAspect.java」；
  ✓「他用 MobaXterm 连客户服务器做部署排查」）
- 一次性行为：某天用了一次某工具、看了一部片，不等于「常用」「喜欢」，反复出现才算

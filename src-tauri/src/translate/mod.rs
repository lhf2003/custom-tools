    ...
    tauri::async_runtime::spawn(async move {
        match capture_selection_text(&app_handle).await {
            Ok(source) => {
                ...
                show_translate_toast(&app_handle, req_id, &source, &target_lang);
...
/// target_lang 缺省取设置值；视图可传覆盖值。返回本次请求 id（前端按 id 过滤事件）。
#[tauri::command]
pub async fn translate_text(
    ...
    let req_id = REQUEST_ID.fetch_add(1, Ordering::Relaxed) + 1;
    stream_translate(&app_handle, req_id, &text, &target_lang).await?;
    Ok(req_id)
}
...
async fn capture_selection_text(app_handle: &AppHandle) -> Result<String, String> {
    ...
    // 3. 轮询剪贴板直到内容变化（SendInput 经消息队列注入，需等待；300ms 无变化判无选区）
...
fn simulate_ctrl_c() {
    ...
    unsafe {
        // 高位为 1 表示按键处于按下态（与 lib.rs 的 VK_LBUTTON 检测同款写法）
        let shift_held = (GetAsyncKeyState(VK_SHIFT.0 as i32) as i16) < 0;
...
/// 弹出翻译浮窗并下发 start 事件（原文 + 目标语言）。位置：鼠标右下方，贴近选区。
fn show_translate_toast(app_handle: &AppHandle, req_id: u64, source: &str, target_lang: &str) {
    ...
    if let Some((cx, cy)) = crate::get_cursor_pos() {
...
/// 无选区/捕获失败提示：复用浮窗，短显后消失（前端 1.5s 后 invoke 隐藏）
